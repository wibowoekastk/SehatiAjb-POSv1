<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Nota PDF</title>

    <style>
        body {
            font-family: 'Arial', sans-serif;
            font-size: 14px;
        }
        table.data td,
        table.data th {
            border: 1px solid #ccc;
            padding: 5px;
        }
        table.data {
            border-collapse: collapse;
        }
        .text-center {
            text-align: center;
        }
        .text-right {
            text-align: right;
        }
    </style>
</head>
<body>
    {{-- KOP NOTA --}}
    <table width="100%">
        <tr>
            <td rowspan="3" width="60%">
                <img src="{{ public_path($setting->path_logo) }}" alt="{{ $setting->path_logo }}" width="120">
                <br>
                {{ $setting->alamat }}
            </td>
            <td>Tanggal</td>
            <td>: {{ tanggal_indonesia(date('Y-m-d')) }}</td>
        </tr>
        <tr>
            <td>Kepada Yth</td>
            <td>: {{ $penjualan->member->nama ?? 'Pelanggan Umum' }}</td>
        </tr>
    </table>

    <br>

    {{-- TABEL RINCIAN BARANG --}}
    <table class="data" width="100%">
        <thead>
            <tr>
                <th>No</th>
                <th>Kode</th>
                <th>Nama Barang</th>
                <th>Kadar</th>
                <th>Berat (gr)</th>
                <th>Harga Satuan</th>
                <th>Jumlah</th>
                <th>Subtotal</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($detail as $key => $item)
                <tr>
                    <td class="text-center">{{ $key+1 }}</td>
                    <td>{{ $item->produk->kode_produk }}</td>
                    <td>{{ $item->produk->nama_produk }}</td>
                    <td class="text-center">{{ $item->kadar }}</td>
                    {{-- ================================================================== --}}
                    {{-- PERBAIKAN: Menggunakan (float) untuk menampilkan gram dengan benar --}}
                    {{-- ================================================================== --}}
                    <td class="text-center">{{ (float)$item->gram }}</td>
                    {{-- ================================================================== --}}
                    <td class="text-right">{{ format_uang($item->harga_jual) }}</td>
                    <td class="text-center">{{ format_uang($item->jumlah) }}</td>
                    <td class="text-right">{{ format_uang($item->subtotal) }}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr>
                <td colspan="7" class="text-right"><b>Total Harga</b></td>
                <td class="text-right"><b>{{ format_uang($penjualan->total_harga) }}</b></td>
            </tr>
            <tr>
                <td colspan="7" class="text-right"><b>Diskon ({{ $penjualan->diskon }}%)</b></td>
                <td class="text-right"><b>{{ format_uang($penjualan->total_harga * $penjualan->diskon / 100) }}</b></td>
            </tr>
            <tr>
                <td colspan="7" class="text-right"><b>Total Bayar</b></td>
                <td class="text-right"><b>{{ format_uang($penjualan->bayar) }}</b></td>
            </tr>
            <tr>
                <td colspan="7" class="text-right"><b>Diterima</b></td>
                <td class="text-right"><b>{{ format_uang($penjualan->diterima) }}</b></td>
            </tr>
            <tr>
                <td colspan="7" class="text-right"><b>Kembali</b></td>
                <td class="text-right"><b>{{ format_uang($penjualan->diterima - $penjualan->bayar) }}</b></td>
            </tr>
        </tfoot>
    </table>

    <br>

    {{-- BAGIAN TANDA TANGAN --}}
    <table width="100%">
        <tr>
            <td><b>Terima kasih telah berbelanja.</b></td>
            <td class="text-center">
                Hormat Kami,
                <br>
                <br>
                <br>
                ({{ auth()->user()->name }})
            </td>
        </tr>
    </table>
</body>
</html>
